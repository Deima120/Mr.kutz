/**
 * Selector de proveedor con búsqueda remota y alta inline (SupplierFormModal).
 */
...
function supplierLabel(supplier) {
  if (!supplier) return '';
  const tax = supplier.taxId ?? supplier.tax_id;
  return `${supplier.name}${tax ? ` · ${tax}` : ''}`;
}
...
/**
 * @param {string|number} value supplierId
 * @param {(supplierId: string, supplier: object|null) => void} onChange
 */
export default function SupplierPicker({
  ...
  useEffect(() => {
    ...
    const t = requestAnimationFrame(() => triggerRef.current?.focus());
  ...
  const handleKeyDown = (event) => {
    ...
    if (event.key === 'Enter') {
      ...
      const supplier = results[highlightIndex];
      if (supplier) selectSupplier(supplier);
    }
  };

  const displayLabel = selectedSupplier ? supplierLabel(selectedSupplier) : placeholder;
            ...
            <ul className="overflow-y-auto py-1.5 flex-1 min-h-0" role="presentation">
              {loading ? (
                ...
                <li className="px-3.5 py-2 text-sm text-stone-500">
                  {query.trim() ? 'Sin resultados' : 'Escribe para buscar proveedores'}
                </li>
              ) : (
                    ...
                    <li key={supplier.id} role="presentation">
                      ...
                      >
                        <span className="min-w-0 flex-1">
                          <span className="block whitespace-normal break-words">{supplier.name}</span>
                          {tax ? (
                            <span className="block text-[11px] text-stone-500 whitespace-normal break-words">{tax}</span>
                          ) : null}
                        </span>
                        {isSelected ? (
              ...
              <div className="border-t border-stone-100 p-1.5 shrink-0">
                ...
                >
                  ...
                  <span className="truncate">
                    {query.trim() ? `Crear «${query.trim()}»` : 'Crear proveedor nuevo'}
                  </span>
                </button>
              </div>
            ) : null}
    ...
    <>
      <div ref={rootRef} className="group relative min-w-0">
        <button
          ...
          disabled={disabled}
          title={hasValue && selectedSupplier ? displayLabel : undefined}
        ...
        >
          <span className={`min-w-0 flex-1 truncate ${!hasValue || !selectedSupplier ? 'text-stone-400' : ''}`}>
            {displayLabel}
          </span>
          <ChevronDown
