    ...
    <div className="relative max-w-6xl mx-auto p-1 animate-fade-in-up">
      ...
      <div className="bg-white rounded-2xl border border-stone-100 p-5 md:p-6 shadow-card hover:shadow-card-hover transition-all duration-300 relative overflow-hidden">
        ...
        <div className="relative flex flex-col md:flex-row items-center md:items-start justify-between gap-6">
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 text-center sm:text-left w-full">
            ...
            <div className="space-y-4 flex-1">
              ...
              <div className="grid grid-cols-2 sm:flex sm:items-center gap-3 pt-1">
                ...
                {/* Solo aparece si el cliente ha faltado alguna vez: es el dato que
                    sustenta la decisión de inactivarlo desde el listado de clientes. */}
                {noShowAppointments > 0 && (
                  <div className="bg-red-50/60 border border-red-100 rounded-xl px-4 py-2 text-center sm:text-left flex items-center gap-3 transition-colors hover:bg-red-50">
                    <div className="w-8 h-8 rounded-lg bg-red-100/70 flex items-center justify-center text-red-600 shrink-0">
                      <CalendarX className="w-4 h-4" />
                    </div>
                    <div>
                      <span className="block text-[10px] text-red-400 uppercase tracking-wider font-bold">No asistió</span>
                      <span className="text-sm font-extrabold text-red-700">{noShowAppointments}</span>
                    </div>
                  </div>
                )}
              </div>
          ...
          {/* Botones de Acción */}
          <div className="flex flex-col sm:flex-row gap-2.5 w-full md:w-auto md:self-start md:mt-1 md:ml-3 shrink-0">
            <Link
              to={`/clients/${id}/edit`}
              className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-stone-900 hover:bg-stone-800 text-gold hover:text-gold-light border border-stone-800 hover:border-gold/45 rounded-xl font-semibold shadow-sm transition-all duration-300 transform active:scale-95 group text-sm"
            >
