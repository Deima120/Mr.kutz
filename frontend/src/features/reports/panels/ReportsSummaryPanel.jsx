/**
 * Resumen ejecutivo (antes ReportsPage completo).
 */
    ...
    return (
      <div className="flex items-center justify-center min-h-[200px]">
        <div className="text-stone-500">
          {loading ? 'Cargando resumen…' : 'No hay datos para este periodo.'}
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <AdminFilterRow>
        ...
        />
        <AdminExportButtons
          onExcel={() => downloadReportExcel(report, { businessName, dateFrom, dateTo })}
          onPdf={() => downloadReportPDF(report, { businessName, dateFrom, dateTo })}
        />
      </AdminFilterRow>
      ...
      >
        {r.count > 0 ? (
          <div className="grid gap-6 md:grid-cols-[240px_1fr]">
                ...
                return (
                  <div key={stars} className="flex items-center gap-2 text-xs">
                    ...
                    <div className="flex-1 bg-stone-100 rounded-full h-2 overflow-hidden">
                      <div className="h-full bg-gold" style={{ width: `${w}%` }} aria-hidden />
                    </div>
            ...
            <ul className="space-y-3">
              {(r.recent || []).slice(0, 6).map((x) => (
                <li key={x.appointmentId} className="border border-stone-200 rounded-xl p-3 text-sm">
                  ...
                  {x.comment && <p className="text-stone-600">&quot;{x.comment}&quot;</p>}
          ...
          <p className="text-stone-500 text-sm">
            Cuando marques citas como completadas, el cliente recibirá un correo para dejar su
            reseña y aparecerá aquí.
          </p>
        )}
      </DataCard>
    </div>
