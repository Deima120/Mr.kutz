...
export function AdminFormCardHeader({ eyebrow, title }) {
  return (
    <header className="shrink-0 flex flex-wrap items-start justify-between gap-3 min-w-0">
      <div className="min-w-0 flex-1">
        {eyebrow && (
          <p className="text-[10px] font-semibold tracking-[0.28em] text-gold mb-1">{eyebrow}</p>
        )}
        <h1 className="font-serif text-xl sm:text-2xl xl:text-[1.65rem] text-stone-900 font-medium tracking-tight break-words">
          {title}
        </h1>
      </div>
...
export function AdminFormPrimaryButton({ children, disabled, type = 'submit', onClick, className = '' }) {
  return (
    <button
      type={type}
      onClick={onClick}
